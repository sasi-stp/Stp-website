    ...
    rowCard.innerHTML = `
        ...
        <div class="form-group-row-three">
            <div class="form-group">
                ...
                <small class="row-stock-lbl" style="color: blue; font-weight: bold; margin-top: 2px; font-size:0.65rem;">තොගය: 0</small>
            </div>
            <div class="form-group">
                <label>Free:</label>
...
function updateLiveTotal() {
    ...
    for(let row of rows) {
        ...
        stockLbl.textContent = `තොගය: ${avail}`;

        const scheme = productsMap[item];
        if(scheme && scheme[2] > 0 && scheme[3] > 0) {
            schemeLbl.textContent = `💡 Free: ${scheme[2]} කට ${scheme[3]} ක්`;
        } else {
...
function renderLiveInvoicePreview(record) {
    ...
    billPreviewBox.innerHTML = `
        <div class="bill-invoice-container">
            ...
            <div class="invoice-meta-row"><span><b>දිනය:</b> ${record.date}</span> <span><b>ID:</b> #${record.id.toString().slice(-6)}</span></div>
            <div class="invoice-meta-row"><span><b>වෙළඳසැල:</b> ${record.shop}</span></div>
            <div class="invoice-meta-row"><span><b>ක්‍රමය:</b> <span style="color:${record.mode === 'Cash'?'green':'purple'}; font-weight:bold;">${record.mode.toUpperCase()}</span></span></div>
            <div class="invoice-divider"></div>
            <table class="invoice-table">
                <thead>
                    <tr><th>විස්තරය</th><th>Qty</th><th>මිල</th><th style="text-align:right;">මුදල</th></tr>
                </thead>
                <tbody>${rowsHtml}</tbody>
            </table>
            <div class="invoice-total-row">
                <span>ශුද්ධ එකතුව (NET):</span>
                <span>රු. ${record.total.toFixed(2)}</span>
            </div>
            <div class="invoice-divider" style="margin-top:15px;"></div>
            <div style="text-align:center; font-size:0.75rem; font-weight:bold; color:var(--primary-color);">ස්තූතියි! 🙏</div>
        </div>
    `;
}
    ...
    salesData.slice().reverse().forEach(s => {
        ...
        salesTableBody.innerHTML += `
            <tr>
                <td>${s.date}</td>
                <td><b>${s.shop}</b></td>
                <td style="font-size: 0.75rem; max-width: 200px; line-height: 1.3;">${descHtml}</td>
...
window.editStockRecord = function(id) {
    ...
    document.getElementById('stock-form-title').textContent = "✏️ තොගය සංස්කරණය";
...
function renderStockOverview() {
    ...
    Object.keys(productsMap).forEach(item => {
        ...
        salesData.forEach(s => {
            if(s.itemsList && Array.isArray(s.itemsList)) {
                s.itemsList.forEach(si => { if(si.item === item) sold += si.qty; });
            }
        });
...
window.deleteStockHistoryRecord = function(id) {
    if(confirm("මෙම සටහන මැකීමට අවශ්‍යද?")) {
...
function updateFilteredAnalytics() {
    ...
    let filteredSales = salesData.filter(s => {
        ...
        if(prod !== "ALL") {
            if(s.itemsList && Array.isArray(s.itemsList)) {
                if(!s.itemsList.some(si => si.item === prod)) return false;
            } else return false;
        }
    ...
    let totalSoldQty = 0, totalIncome = 0, totalRetQty = 0, totalReturnLoss = 0, totalProductionCost = 0, outstandingIncurred = 0;
    ...
    filteredSales.forEach(s => {
        ...
        if(prod === "ALL" && s.mode === 'Credit') outstandingIncurred += s.total;
    });

    let filteredExpenses = expenses.filter(e => {
        const eDate = new Date(e.date);
        if(time === "daily") return eDate.toDateString() === now.toDateString();
        if(time === "weekly") return eDate >= new Date(now.getTime() - 7*24*60*60*1000);
...
window.closeShopPhoneModal = function() { document.getElementById('edit-phone-modal').classList.add('hidden'); };
window.saveShopPhoneModal = function() {
    const idx = parseInt(document.getElementById('modal-shop-index').value);
    shopDirectory[idx].phone = document.getElementById('modal-phone-input').value.trim();
    localStorage.setItem('watalappan_shop_directory', JSON.stringify(shopDirectory));
